import request from 'supertest';
import express from 'express';
import { jest } from '@jest/globals';
import { mockDeep, mockReset } from 'jest-mock-extended';
import { PrismaClient } from '@prisma/client';

// 1. Cria o mock do banco
const prismaMock = mockDeep<PrismaClient>();

// 2. Intercepta a importação do Prisma
jest.unstable_mockModule('../../src/prisma.js', () => ({
  __esModule: true,
  prisma: prismaMock,
}));

// 3. Importa dinamicamente as rotas e o handler após o mock ter sido injetado
// O uso do [try/catch] previne que o Jest exploda o arquivo inteiro se os arquivos ainda não existirem
let driverRoutes: any;
let errorHandler: any;
try {
  const routesModule = await import('../../src/routes/driver.routes.js');
  driverRoutes = routesModule.driverRoutes;
  const errorModule = await import('../../src/middlewares/errorHandler.js');
  errorHandler = errorModule.errorHandler;
} catch (e) {
  // Ignora o erro na fase vermelha (Red) do TDD caso os arquivos ainda não tenham sido criados
}

describe('Driver Controller Integration Tests', () => {
  let app: express.Express;

  beforeAll(() => {
    app = express();
    app.use(express.json());
    
    // Associa as rotas se elas existirem
    if (driverRoutes) app.use('/drivers', driverRoutes);
    // Adiciona o middleware de erro se ele existir
    if (errorHandler) app.use(errorHandler);
  });

  beforeEach(() => {
    mockReset(prismaMock);
  });

  describe('POST /drivers', () => {
    it('deve retornar 201 e criar o motorista com payload válido', async () => {
      // Mock do findUnique: não encontrou a CNH
      prismaMock.driver.findUnique.mockResolvedValue(null);
      // Mock do create
      prismaMock.driver.create.mockResolvedValue({
        id: '123',
        name: 'Mario',
        document: '12345678901',
        licenseNumber: '99999999999',
        status: 'AVAILABLE',
        createdAt: new Date(),
        updatedAt: new Date()
      });

      const response = await request(app)
        .post('/drivers')
        .send({
          name: 'Mario',
          document: '12345678901',
          licenseNumber: '99999999999',
          status: 'AVAILABLE',
          disponivel: true
        });

      expect(response.status).toBe(201);
      expect(response.body).toHaveProperty('id', '123');
      expect(response.body.name).toBe('Mario');
    });

    it('deve retornar 400 se faltar campos (Erro de Validação Zod)', async () => {
      const response = await request(app)
        .post('/drivers')
        .send({
          name: 'Mario' // Faltam campos obrigatórios
        });

      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty('message', 'Erro de Validação (Zod)');
      expect(response.body.errors).toBeDefined();
    });

    it('deve retornar 400 se a CNH já estiver cadastrada', async () => {
      // Mock do findUnique: achou um motorista existente
      prismaMock.driver.findUnique.mockResolvedValue({
        id: '123',
        name: 'Luigi',
        document: '12345678901',
        licenseNumber: '99999999999',
        status: 'AVAILABLE',
        createdAt: new Date(),
        updatedAt: new Date()
      });

      const response = await request(app)
        .post('/drivers')
        .send({
          name: 'Mario Novo',
          document: '12345678901',
          licenseNumber: '99999999999',
          status: 'AVAILABLE',
          disponivel: true
        });

      expect(response.status).toBe(409);
      expect(response.body.message).toBe('CNH já cadastrada no sistema');
    });
  });

  describe('GET /drivers', () => {
    it('deve retornar 200 e uma lista de motoristas', async () => {
      prismaMock.driver.findMany.mockResolvedValue([
        {
          id: '123',
          name: 'Mario',
          document: '12345678901',
          licenseNumber: '99999999999',
          status: 'AVAILABLE',
          createdAt: new Date(),
          updatedAt: new Date()
        }
      ]);

      const response = await request(app).get('/drivers');

      expect(response.status).toBe(200);
      expect(response.body).toBeInstanceOf(Array);
      if (response.body.length > 0) {
        expect(response.body[0].name).toBe('Mario');
      }
    });
  });
});
